package com.sajidur.diugo.Backend;

import java.util.ArrayList;

public class DataHold {


    //All Constant Variables are here
    public static final int Labs=1;

    public static final int Computers=2;

    public static final int TopLevelManagement=1;

    public static final  int Director=2;






    public  static String User="sajidursajidurm@gmail.com";
    public static ArrayList<Computers> computersArrayList;

    public static ArrayList<Labs> labsArrayList;

    public static ArrayList<ComputerBookingInfo> computerBookingInfosArrayList;

    public static ArrayList<Employee> employeeArrayList;

    public static String Date;
    public static String ActivityHeader;
    public static int DataGetsFor;

    public static  int LabNo;

    public  static  int ComputerID;
}
